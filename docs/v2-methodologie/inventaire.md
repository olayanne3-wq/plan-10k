# Inventaire de l'application "Yoria" (anciennement "Run by Léa")

> Vue d'ensemble de référence — à relire en début de session pour retrouver le contexte
> sans re-parcourir tout le repo. Mis à jour au 14 juillet 2026 (**rebranding complet
> "Run by Léa" → "Yoria"**, palette graphique, thème clair/sombre avec bascule
> utilisateur, recoloration des badges par intensité, voir §13-§14 ; **bug
> `v2_gist_id` de synchronisation Supabase résolu**, voir §15 ; chantier ACWR en cours ;
...
> **publication Play Store — `.aab` uploadé, app testée et installée en piste
> de test interne**, voir §11 ; **Mode Forme (v2.6)
> câblé de bout en bout**, voir §12 ; **bug TWA duplication de tâches résolu**, voir §12.7).
...
**Yoria** (anciennement "Run by Léa", renommée le 14 juillet 2026 — voir §13) — PWA
coach running, génère des plans d'entraînement adaptatifs. Déployée sur Vercel :
`plan-10k-alpha.vercel.app` (domaine inchangé malgré le rebranding). Repo GitHub :
`olayanne3-wq/yoria` (renommé depuis `plan-10k`, redirect GitHub automatique en place).
...
## 9. État des chantiers (au 14/07/2026)
...
| **Publication Play Store (TWA)** | 🟡 **En cours** (14 juillet) — `.aab` uploadé, app testée en piste de test interne ; reste captures d'écran à confirmer, pas de passage en production prévu — voir §11 |
| Mode Forme (v2.6) | ✅ Câblé de bout en bout (13 juillet) — reste `genererBlocSuivant()` à déclencher côté `index.html`, voir §12.7 |
| Bug TWA duplication de tâches (`target="_blank"`) | ✅ Clos (13 juillet) — voir §12.7 |
| Rebranding "Run by Léa" → "Yoria" (nom, palette, thème clair/sombre, badges par intensité) | ✅ Clos (14 juillet) — voir §13-§14 |
| Bug `v2_gist_id` absent de la sync Supabase (plans invisibles sur nouvel appareil) | ✅ Clos (14 juillet) — voir §15 |
...
- ✅ Compte développeur Google Play créé, 25$ payés, **vérification
  d'identité validée** (mise à jour du 14 juillet — était "en cours" au
  13 juillet)
- ✅ Politique de confidentialité rédigée (à déployer sur `public/privacy.html`)
- ✅ Textes de fiche store rédigés **et remplis dans Play Console** (mise à
  jour du 14 juillet)
- ✅ Feature graphic composé et validé (version horizontale)
- ✅ Classification du contenu **remplie** (questionnaire Play Console, mise
  à jour du 14 juillet)
- ✅ Data Safety form **rempli** dans Play Console (mise à jour du 14 juillet)
- ✅ App **créée** dans Play Console (mise à jour du 14 juillet)
- ✅ Upload du `.aab` **fait** (mise à jour du 14 juillet) — `.aab` régénéré
  avec la nouvelle identité Yoria (cf. §13.2 pour la régénération de l'APK
  déjà faite suite au rebranding) et soumis dans Play Console
- 🔜 Captures d'écran (à prendre directement sur le téléphone, pas encore
  confirmé fait — à vérifier avec Laurent, incertain si couvert par "fiche
  store remplie" ci-dessus)
- ✅ Test en piste interne **fait** (mise à jour du 14 juillet) — app
  **testée et installée en conditions réelles** en piste de test interne.
  C'est la piste retenue, pas de passage en production prévu pour
  l'instant (cf. décision de diffusion ci-dessus)
...
`index.html` :
- EF, LONGUE → turquoise (`--accent2`)
- SEUIL, SPEC/allure course → bleu (`--accent`)
- VMA, TEST, RACE → orange (`--warn`)
...
## 15. Bug `v2_gist_id` manquant de la synchronisation Supabase (résolu le 14/07/2026)
...
**Diagnostic** — mené en direct dans les DevTools du navigateur
(`await chargerPlans()` → `[]`, puis vérification manuelle de chaque clé
...
**Correctif appliqué** :
1. Nouvelle colonne `v2_gist_id` (text) ajoutée à la table `integrations`
   côté Supabase (`ALTER TABLE integrations ADD COLUMN IF NOT EXISTS v2_gist_id text;`)
2. `v2_gist_id` ajoutée à `CLES_INTEGRATIONS` et à la table de routage
...
3. Contournement manuel appliqué en urgence sur l'appareil de test avant
...
Android (app Play Store, déconnexion/reconnexion), après application des
deux correctifs ci-dessus — les deux ensemble résolvent le problème
complètement.
