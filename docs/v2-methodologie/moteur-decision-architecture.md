...
## 13. État d'implémentation réel (16 juillet 2026)

Les modules 1 (partiel) et 5 ont été implémentés, testés et déployés en production le 16 juillet 2026 — première traversée complète du plan de conception vers du code réel. Le détail complet (fichiers livrés, écarts avec ce document, bug structurel trouvé et corrigé, validation sur données réelles, prochaines étapes) est documenté dans le **document d'intégration, §11**, pour éviter de dupliquer l'information à deux endroits qui risqueraient de diverger.

En résumé, pour ce document :

- **Module 1** : seul `RunnerStateCalculator` (charge/fatigue/ACWR/confiance) et `EngagementCalculator` (régularité comportementale) sont codés. `GoalFeasibilityCalculator` n'existe pas. Aucun des deux calculateurs codés n'implémente le contrat complet du §3.3/§6 — voir le tableau d'écarts en §11.2 du document d'intégration.
